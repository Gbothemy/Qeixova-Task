...
export async function GET() {
  ...
  const [tasks, completions] = await Promise.all([
    sql`SELECT COUNT(*)::int AS total, COUNT(CASE WHEN is_active THEN 1 END)::int AS active FROM tasks WHERE business_id = ${session.businessId}`,
    sql`
      SELECT
        ...
        COALESCE(SUM(CASE WHEN c.status = 'approved' THEN t.reward ELSE 0 END), 0)::int AS total_qlт_spent
      FROM completions c
